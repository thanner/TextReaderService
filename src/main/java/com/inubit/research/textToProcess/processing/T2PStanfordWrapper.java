...
public class T2PStanfordWrapper {
     ...
     * @param fakeVar
     */
    public T2PStanfordWrapper(boolean fakeVar) {
        if (f_parser == null) {
            try {
                InputStream is = this.getClass().getClassLoader().getResourceAsStream("englishFactored.ser.gz");
            ...
            f_parser.setOptionFlags("-retainTmpSubcategories");
    ...
    public T2PStanfordWrapper() {
        if (f_parser == null) {
            try {
                ObjectInputStream in;
                InputStream is;
                URL u = T2PStanfordWrapper.class.getClassLoader().getResource("/englishFactored.ser.gz");
                if (u == null) {
                    //opening from IDE
