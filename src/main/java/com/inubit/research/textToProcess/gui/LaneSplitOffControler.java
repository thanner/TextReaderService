...
public class LaneSplitOffControler implements ActionListener, WorkbenchEditorListener {
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (_lane != null) {
            ...
            //TODO ExampleApp was sort Clusters
    ...
    private void transformToMessageFlow(BPMNModel model, ProcessEdge e) {
        ...
        if (f_createMsgEvt || !(_source instanceof Task) || _source.getStereotype().length() > 0 || _ee) {
            _start = _ee ? new MessageEndEvent() : new MessageIntermediateEvent();
            if (!_ee)
                _start.setProperty(MessageIntermediateEvent.PROP_EVENT_SUBTYPE, MessageIntermediateEvent.EVENT_SUBTYPE_THROWING);
        ...
        if (f_createMsgEvt || !(_target instanceof Task) || _target.getStereotype().length() > 0 || _se) {
