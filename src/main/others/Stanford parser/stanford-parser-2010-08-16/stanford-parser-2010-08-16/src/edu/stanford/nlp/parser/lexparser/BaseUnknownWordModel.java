    ...
    for (Tree t : trees) {
      ...
      for (Tree node : t) {
        if (node.isPreTerminal()) {
          ...
          //if (ExampleApp.verbose) EncodingPrintWriter.out.println("recording instance of " + wt.toString(),encoding); // testing

          wtCount.incrementCount(wt);// TaggedWord has crummy equality conditions
          //if (ExampleApp.verbose) EncodingPrintWriter.out.println("This is the " + wtCount.getCount(wt) + "th occurrence of" + wt.toString(),encoding); // testing
          tagCount.incrementCount(tag);
          //boolean alreadySeen = seenWords.add(word);
          seenWords.add(word);
          // if (ExampleApp.verbose) if(! alreadySeen) EncodingPrintWriter.out.println("already seen " + wt.toString(),encoding); // testing
        }
      }
    }
