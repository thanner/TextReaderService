/*  $Id: bv.pl,v 1.3 2005/02/25 12:30:22 toms Exp $
...
% deref(Lin,Lind)
%
% Makes a linear equation of the form [v(I,[])|H] into a solvable linear equation.
...
% the length of the part to convert and [Vs|VsTail] is a difference list containing
...
% deref_var(X,Lin)
% 
% Returns the equation of variable X. If X is a new variable, a new equation X = X is made.

deref_var(X,Lin) :-
	get_attr(X,itf3,(_,_,lin(Lin),_)),
	!.
deref_var(X,Lin) :-	% create a linear var
	Lin = [0.0,0.0,l(X*1.0,Ord)],
	put_attr(X,itf3,(type(t_none),strictness(0),lin(Lin),order(Ord),n,n,n,n,n,n)).
...
%
% Makes Lin the linear equation of new variable Var, makes all variables of Lin, and Var
% of the same class and bounds Var by type(Type) and strictness(Strictness)

var_with_def_intern(Type,Var,Lin,Strict) :-
	put_attr(Var,itf3,(type(Type),strictness(Strict),lin(Lin),order(_),n,n,n,n,n,n)),	% check uses
	Lin = [_,_|Hom],
...
var_intern(Type,Var,Strict) :-
	Lin = [0.0,0.0,l(Var*1.0,Ord)],
	put_attr(Var,itf3,(type(Type),strictness(Strict),lin(Lin),order(Ord),n,n,n,n,n,n)),
...
var_intern(Var,Class) :-			  % for ordered/1 but otherwise free vars
	get_attr(Var,itf3,(type(_),_,lin(_),_)),
	!,
	get_or_add_class(Var,Class).	
var_intern(Var,Class) :-
	put_attr(Var,itf3,(type(t_none),strictness(0),lin([0.0,0.0,l(Var*1.0,Ord)]),order(Ord),n,n,n,n,n,n)),
	get_or_add_class(Var,Class).
...
'solve_=\\='(Nf) :-
	...
	;
	  Hom = [_|_] ->
		...
		var_with_def_intern(t_none,Nz,Lind,0), % make new variable Nz = Lind
		get_attr(Nz,itf3,(Ty,St,Li,Or,Cl,Fo,_,RAtt)),
		put_attr(Nz,itf3,(Ty,St,Li,Or,Cl,Fo,nonzero,RAtt))	% make Nz nonzero
	).
...
inf(Expression,Inf,Vector,Vertex) :-
	% wait until Expression becomes linear, Nf contains linear Expression in normal form
...
inf_lin(Lin,_,Vector,_) :-
	...
	(
	  blackboard_inf(_) ->
		...
		retract(blackboard_inf(_)),
		assert(blackboard_inf([Inf|Values]))
	;
		assert(blackboard_inf([Inf|Values]))
	),
	fail.
inf_lin(_,Infimum,_,Vertex) :-
	retract(blackboard_inf(L)),
...
iterate_dec(OptVar,Opt) :-
	get_attr(OptVar,itf3,(_,_,lin(Lin),_)),
	Lin = [I,R|H],
	dec_step(H,Status),
	(
	  Status = applied ->
...
dec_step([l(V*K,OrdV)|Vs],Status) :-
	get_attr(V,itf3,(type(W),_,_,_,class(Class),_)),
	(
	  W = t_U(U) ->
		...
		(
		  K > 1e-010 ->		% K > 0
			...
			(
			  lb(Class,OrdV,Vub-Vb-_) ->	% found a lower bound
...
inc_step([l(V*K,OrdV)|Vs],Status) :-
	get_attr(V,itf3,(type(W),_,_,_,class(Class),_)),
	(
	  W = t_U(U) ->	% active upperbound
	
		(
		  K < -1e-010 ->	% K < 0: try to find lower bound for V
		...
		;			% K > 0: no way to increment
			inc_step(Vs,Status)
 		)
	;
	  W = t_lU(L,U) ->	% active upperbound, passive lowerbound
		
		(
		  K < -1e-010 ->	% K < 0, try to find lower bound
...
ub_first([Dep|Deps],OrdX,Tightest) :-
	(
	  get_attr(Dep,itf3,(type(Type),_,lin(Lin),_)),
...
ub([Dep|Deps],OrdX,T0,T1) :-
	(
	  ...
	  ub_inner(Type,OrdX,Lin,W,Ub),
	  T0 = _-Ubb,
	  Ub - Ubb < -1e-010,	% Ub < Ubb: tighter upper bound is a smaller one
	  Ub > -1e-010 -> 	% Ub >= 0: upperbound should be larger than 0; rare failure
...
ub_inner(t_l(L),OrdX,Lin,t_L(L),Ub) :-
	nf_rhs_x(Lin,OrdX,Rhs,K), % Rhs is right hand side of lin. eq. Lin containing term X*K
	K < -1e-010,	% K < 0
	Ub is (L-Rhs)/K.
...
ub_inner(t_lu(L,U),OrdX,Lin,W,Ub) :-
	nf_rhs_x(Lin,OrdX,Rhs,K),
	(
	  K < -1e-010 ->	% K < 0, use lowerbound

		W = t_Lu(L,U),
		Ub = (L-Rhs)/K
		
	;
	  K > 1e-010 ->		% K > 0, use upperbound
...
% lowering the bound for X more, W the actual bound that has to be activated and
% Lb the amount that the upperbound can be lowered.
...
%
% Returns in Tightest how much we can lower the upperbound of X without violating
% a bound of Deps.
% Tightest has the form Dep-W-Lb with Dep the variable whose bound is violated when
% lowering the bound for X more, W the actual bound that has to be activated and
% Lb the amount that the upperbound can be lowered. X has ordering attribute OrdX.

lb_first([Dep|Deps],OrdX,Tightest) :-
	(
	  get_attr(Dep,itf3,(type(Type),_,lin(Lin),_)),
	  lb_inner(Type,OrdX,Lin,W,Lb),
	  Lb < 1e-010 ->	% Lb =< 0: Lb > 0 means a violated bound
...
%
% See lb_first/3: this one does the same thing, but is used for the steps after the first one
% and remembers the tightest bound so far.
...
lb([Dep|Deps],OrdX,T0,T1) :-
	(
	  ...
	  lb_inner(Type,OrdX,Lin,W,Lb),
	  T0 = _-Lbb,
	  Lb - Lbb > 1e-010,		% Lb > Lbb: choose the least lowering, others might violate bounds
	  Lb < 1e-010 ->		% Lb =< 0: violation of a bound (without lowering)
...
% So a lowerbound of X (which has scalar K in Lin), could be at most (L-(I+R))/K
% lower than its upperbound (if K is positive).
% Also note that Lb should always be smaller than 0, otherwise the row isn't feasible.
% X has ordering attribute OrdX.

lb_inner(t_l(L),OrdX,Lin,t_L(L),Lb) :-
	nf_rhs_x(Lin,OrdX,Rhs,K),	% if linear equation Lin contains the term X*K,
					% Rhs is the right hand side of that equation
	K > 1e-010,	% K > 0
	Lb is (L-Rhs)/K.
...
solve([],_,I,Bind0,Bind0) :- 
	!,
	I >= -1e-010,	% I =:= 0: redundant or trivially unsat
	I =< 1e-010.
solve(H,Lin,_,Bind0,BindT) :-
	sd(H,[],ClassesUniq,9-9-0,Category-Selected-_,NV,NVT),
	get_attr(Selected,itf3,(_,_,_,order(Ord),_)),
	isolate(Ord,Lin,Lin1),	% Lin = 0 => Selected = Lin1
	(
	  Category = 1 ->	% classless variable, no bounds 
		
		get_attr(Selected,itf3,(Ty,St,_,RAtt)),
		put_attr(Selected,itf3,(Ty,St,lin(Lin1),RAtt)),
		Lin1 = [Inhom,_|Hom],
	...
	;
	  Category = 2 ->	% class variable, no bounds
		...
		get_attr(Selected,itf3,(_,_,_,_,class(NewC),_)),
		...
		eq_classes(NV,NVT,ClassesUniq)
	;
	  Category = 3 ->	% classless variable, all variables in Lin and Selected are bounded
		
		get_attr(Selected,itf3,(type(Type),St,_,RAtt)),
		put_attr(Selected,itf3,(type(Type),St,lin(Lin1),RAtt)),
     		deactivate_bound(Type,Selected),	
		eq_classes(NV,NVT,ClassesUniq),
		basis_add(Selected,Basis),
		undet_active(Lin1),	% we can't tell which bound will likely be a problem at this point
		Lin1 = [Inhom,_|Hom],
		bs_collect_binding(Hom,Selected,Inhom,Bind0,Bind1),	% only if Hom = []
		rcbl(Basis,Bind1,BindT)	% reconsider entire basis
	;
	  Category = 4 ->	% class variable, all variables in Lin and Selected are bounded
      		
		get_attr(Selected,itf3,(type(Type),_,_,_,class(NewC),_)),
		class_allvars(NewC,Deps),
		(
		  ClassesUniq = [_] ->		% rank increasing
...
solve_x(H,Lin,_,X,Bind0,BindT) :-
	...
	get_attr(X,itf3,(_,_,_,order(OrdX),_)),
	isolate(OrdX,Lin,Lin1),
	(
	  get_attr(X,itf3,(_,_,_,_,class(NewC),_)) ->
		...
		eq_classes(NV,NVT,ClassesUniq)
	;
      		get_attr(X,itf3,(Ty,St,_,RAtt)),
		put_attr(X,itf3,(Ty,St,lin(Lin1),RAtt)),
		Lin1 = [Inhom,_|Hom],
...
%
% Does the same thing as solve_x/2, but has the ordering of X and its class as input
% This also means that X has a class which is not sure in solve_x/2
...
solve_ord_x([_|_],Lin,_,OrdX,ClassX,Bind0,BindT) :-
	isolate(OrdX,Lin,Lin1),
	Lin1 = [_,_|H1],
	sd(H1,[],ClassesUniq1,9-9-0,_,NV,NVT),	% do sd on Lin without X, then add class of X
...
%
% ClassesOut is a sorted list of the different classes that are either in ClassesIn or
% that are the classes of the variables in Hom. Variables that do not belong to a class
% yet, are put in the difference list NV.

sd([],Class0,Class0,Preference0,Preference0,NV0,NV0).
sd([l(X*K,_)|Xs],Class0,ClassN,Preference0,PreferenceN,NV0,NVt) :-
	(
	  get_attr(X,itf3,(_,_,_,_,class(Xc),_)) ->			% old: has class

		NV0 = NV1,
		ord_add_element(Class0,Xc,Class1),
		(
		  get_attr(X,itf3,(type(t_none),_)) ->

			preference(Preference0,2-X-K,Preference1)	% has class, no bounds => category 2
		;
	    		preference(Preference0,4-X-K,Preference1)	% has class, is bounded => category 4
		)
  	;								% new: has no class
		Class1 = Class0,
		...
		(
		  get_attr(X,itf3,(type(t_none),_)) ->
			...
			preference(Preference0,1-X-K,Preference1)	% no class, no bounds => category 1
		;
			preference(Preference0,3-X-K,Preference1)	% no class, is bounded => category 3
		)
  	),
...
preference(A,B,Pref) :-
	...
	(
	  Rel = (=) ->
		...
		Pref = B   % ( abs(Ka)=<abs(Kb) -> Pref=A ; Pref=B )
  	;
	  Rel = (<) ->
...
%
% Attaches all classless variables NV to a new class and equates all other classes
% with this class. The equate operation only happens after attach_class because the
% unification of classes can bind the tail of the AllVars attribute to a nonvar and
% then the attach_class operation wouldn't work.
...
eq_classes(NV,NVT,Cs) :-
	class_new(Su,NV,NVT,[]), % make a new class Su with NV as the variables
...
attach_class([X|Xs],Class) :-
	get_attr(X,itf3,(Ty,St,Li,Or,_,RAtt)),
	put_attr(X,itf3,(Ty,St,Li,Or,class(Class),RAtt)),
	attach_class(Xs,Class).

% unconstrained(Lin,Uc,Kuc,Rest)
%
% Finds an unconstrained variable Uc (type(t_none)) in Lin with scalar Kuc and removes it
% from Lin to return Rest.

unconstrained(Lin,Uc,Kuc,Rest) :-
	Lin = [_,_|H],
	sd(H,[],_,9-9-0,Category-Uc-_,_,_),
	Category =< 2,
	get_attr(Uc,itf3,(_,_,_,order(OrdUc),_)),
...
% get_or_add_class(X,Class)
%
% Returns in Class the class of X if X has one, or a new class where X now belongs to if X didn't have one.

get_or_add_class(X,Class) :-
	get_attr(X,itf3,(_,_,_,_,class(ClassX),_)),
  	!,
...
allvars(X,Allvars) :-
	get_attr(X,itf3,(_,_,_,_,class(C),_)),
	class_allvars(C,Allvars).

% deactivate_bound(Type,Variable)
%
% The Type of the variable is changed to reflect the deactivation of its bounds.
...
deactivate_bound(t_L(L),X) :- 
	get_attr(X,itf3,(_,RAtt)),
	put_attr(X,itf3,(type(t_l(L)),RAtt)).
deactivate_bound(t_Lu(L,U),X) :-
	get_attr(X,itf3,(_,RAtt)),
	put_attr(X,itf3,(type(t_lu(L,U)),RAtt)).
deactivate_bound(t_U(U),X) :-
	get_attr(X,itf3,(_,RAtt)),
	put_attr(X,itf3,(type(t_u(U)),RAtt)).
deactivate_bound(t_lU(L,U),X) :-
	get_attr(X,itf3,(_,RAtt)),
	put_attr(X,itf3,(type(t_lu(L,U)),RAtt)).
...
%
% Variable X gets new type Type which reflects the activation of a bound with value
% Value. In the linear equations of all the variables belonging to the same class as 
% X, X is substituted by [0,Value,X] to reflect the new active bound.

intro_at(X,Value,Type) :-
	get_attr(X,itf3,(_,St,Li,order(Ord),class(Class),RAtt)),
	put_attr(X,itf3,(type(Type),St,Li,order(Ord),class(Class),RAtt)),
	(
	  Value >= -1e-010,	% Value =:= 0
	  Value =< 1e-010 ->
...
% For the case of t_lu(_,_) the lower or upper bound is activated depending on K and S:
...
%
detach_bounds(V) :-
	get_attr(V,itf3,(_,_,lin(Lin),order(OrdV),class(Class),RAtt)),
	put_attr(V,itf3,(type(t_none),strictness(0),lin(Lin),order(OrdV),class(Class),RAtt)),
	(
	  indep(Lin,OrdV) ->
		
		( 
		  ub(Class,OrdV,Vub-Vb-_) ->	% exchange against thightest
...
basis_drop(X) :-
	get_attr(X,itf3,(_,_,_,_,class(Cv),_)),
...
%
% NewBasis is the result of adding X to the basis of the class to which X belongs.
...
%
% Removes Leave from the basis of the class to which it belongs, and adds Enter to that basis.

basis_pivot(Leave,Enter) :-
	get_attr(Leave,itf3,(_,_,_,_,class(Cv),_)),
...
% pivot(Dep,Indep)
%
% The linear equation of variable Dep, is transformed into one of variable Indep,
% containing Dep. Then, all occurrences of Indep in linear equations are substituted
...
pivot(Dep,Indep) :-
	get_attr(Dep,itf3,(_,_,lin(H),order(OrdDep),_)),
	get_attr(Indep,itf3,(_,_,_,order(Ord),class(Class),_)),
...
pivot_a(Dep,Indep,Vb,Wd) :-
	...
	get_attr(Indep,itf3,(_,RAtt)),
	put_attr(Indep,itf3,(type(Wd),RAtt)).

pivot_b(Vub,V,Vb,Wd) :-
	(
	  Vub == V ->
 		
		get_attr(V,itf3,(_,St,Li,order(Ord),class(Class),RAtt)),
		put_attr(V,itf3,(type(Vb),St,Li,order(Ord),class(Class),RAtt)),
...
%
% X with ordering attribute OrdX, is substituted in all linear equations of variables in the class Class,
% by linear equation [0,Delta,l(X*1,OrdX)]. This reflects the activation of a bound.
...
%
% X with ordering OrdX is substituted in all linear equations of variables in the class Class,
% by linear equation Lin
...
% bs(Vars,OrdV,Lin)
%
% In all linear equations of the variables Vars, variable V with ordering attribute OrdV 
% is substituted by linear equation Lin.
%
...
bs([X|Xs],OrdV,Lin) :-
	(
	  get_attr(X,itf3,(Ty,St,lin(LinX),RAtt)),
	  nf_substitute(OrdV,Lin,LinX,LinX1) ->
		
		put_attr(X,itf3,(Ty,St,lin(LinX1),RAtt)),
      		bs(Xs,OrdV,Lin)
	;
		bs(Xs,OrdV,Lin)
	).
...
%
% Collects bindings (of the form [X-I] where X = I is the binding) by substituting
% Selected in all linear equations of the variables Deps (which are of the same class),
% by Lin. Selected has ordering attribute SelectedOrd.
%
...
% When a bound on a variable X of Lin gets activated, its value, multiplied with the scalar of X,
% is added to the R component of Lin.
% When we consider the lowerbound of V, it must be smaller than R + I, since R contains at best the
% lowerbounds of the variables in Lin (but could contain upperbounds, which are of course larger).
% So checking this can show the violation of a bound of V. A similar case works for the upperbound.

rcb(X,Status,Violated) :-
	get_attr(X,itf3,(type(Type),_,lin(Lin),_)),
	Lin = [I,R|H],
	(	
	  % case 1: lowerbound: R + I should always be larger than the lowerbound
	  Type = t_l(L) ->
		...
		inc_step(H,Status)
	;
	  % case 2: upperbound: R + I should always be smaller than the upperbound
	  Type = t_u(U) ->
...
rcbl_opt(l(L),X,Continuation,B0,B1) :-
	get_attr(X,itf3,(type(Type),strictness(Strict),lin(Lin),_)),
	Lin = [I,R|_],
	Opt is R+I,
	TestLO is L - Opt,
	(	
...
rcbl_app(l(L),X,Continuation,B0,B1) :-
	...
	Lin = [I,R|H],
	(
	  R + I - L > 1e-010 ->		% R+I > L: within bound now
...
rcbl_app(u(U),X,Continuation,B0,B1) :-
	...
	Lin = [I,R|H],
	(
	  R + I - U < -1e-010 ->	% R+I < U: within bound now
...
dump_var(t_l(L),V,I,H) -->
	!,
	{
	  ...
	  get_attr(V,itf3,(_,strictness(Strict),_)),
	  Sm is Strict /\ 2,
	  Kr is 1.0/K,
	  Li is Kr*(L-I),
	  mult_hom(H,Kr,H1),
...
dump_var(t_lu(L,U),V,I,H) --> 
	...
	dump_var(t_l(L),V,I,H),
	dump_var(t_U(U),V,I,H).
dump_var(T,V,I,H) -->	% shouldn't happen
	[ V:T:I+H ].
...
% FilteredStrictness is the component of strictness related to the bound: 0 means nonstrict,
% 1 means strict upperbound, 2 means strict lowerbound, 3 is filtered out to either 1 or 2.
...
%
% Returns in Dump a representation of the nonzero constraint of variable V which has linear
% equation H + I.
